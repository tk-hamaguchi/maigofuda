...
RSpec.describe Maigofuda::ServiceUnavailableError do
  ...
  describe 'super class' do
    it { expect(described_class.superclass).to eq Maigofuda::Error }
  end

  its(:log_level) { is_expected.to eq :error }
  its(:status_code) { is_expected.to eq 503 }
  ...
  describe '#error_code' do
    ...
    context 'when error_code_table => { "Maigofuda::ServiceUnavailableError" => "U123" }' do
      let(:error_code_table) { { 'Maigofuda::ServiceUnavailableError' => 'U123' } }
  ...
  describe '#localized_message' do
    ...
    context 'without args' do
      ...
      context 'when "en.maigofuda.error_messages.maigofuda.service_unavailable_error" => "Hoge"' do
        let(:translations) { { maigofuda: { error_messages: { maigofuda: { service_unavailable_error: 'Hoge' } } } } }
